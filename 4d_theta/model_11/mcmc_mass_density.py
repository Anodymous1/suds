...
if __name__ == "__main__":
    ...
    test_theta_raw = np.array(pd.read_csv("./model_14/train_theta_model_14.csv", header=None))
    test_x_raw = np.array(pd.read_csv("./model_14/train_x_model_14.csv", header=None))

    t, x = standardize(test_theta_raw, test_x_raw)
    
    x_mass_density = np.array(pd.read_csv(f"./model_14/mass_density_x_{prof}_model_14_s1000.csv", header=None))
    
    test_x_raw = (x_mass_density - x[1])/ x[2]
    ...
    with open(f'./model_14/inference_model_14.pkl', 'rb') as file:
        # Load the object from the file
        inference = pickle.load(file)
    ...
    posterior = inference.build_posterior( 
                                        ...
                                        mcmc_parameters={"warmup_steps":500,
                                                            ...
                                                            "num_workers": 1,
                                                            "init_strategy": "sir",
                                                            "init_strategy_parameters": {"num_candidate_samples": 999},
                                                            "thin": 4})
    ...
    pd.DataFrame(final_samples[0]).to_csv(f"./model_14/mass_density_samples_model_14_{prof}_s1000.csv", header=None, index=None)
