...
def save_posterior(posterior, x_mass_density, prof, i):
    
    with torch.no_grad():
        samples = posterior.sample((2000,), x=x_mass_density, show_progress_bars=True).numpy()

    pd.DataFrame(samples).to_csv(f"./model_15/posterior/mass_density_samples_model_15_{prof}_r{i + 1}.csv", header=None, index=None)
...
# data
theta_train = torch.from_numpy(np.array(pd.read_csv("./model_14/train_theta_model_14.csv", header=None))).float()
x_train = torch.from_numpy(np.array(pd.read_csv("./model_14/train_x_model_14.csv", header=None))).float()
...
# x_o
x_o = torch.tensor(np.array(pd.read_csv(f"./model_15/x_o_{prof}.csv", header=None))).float()
x_o = (x_o - x[1]) / x[2]

# posetrior x
x_mass_density = torch.tensor(np.array(pd.read_csv(f"./model_15/mass_density_x_{prof}_model_15.csv", header=None))).float()
x_mass_density = (x_mass_density - x[1])/ x[2]
...
num_samples = 1000
num_rounds = 5
base_lr = 0.00013819189009962727
...
save_posterior(posterior, x_mass_density, prof, -1)
...
for i in range(num_rounds):
    ...
    save_posterior(posterior, x_mass_density, prof, i)
...
# save model
with open(f"./model_15/inference_model_15_{prof}.pkl", "wb") as handle:
