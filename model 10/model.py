...
theta_train_raw = np.array(pd.read_csv("./model 3/training_theta(poisson).csv", header=None))
x_train_raw = np.array(pd.read_csv("./model 3/training_x(poisson).csv", header=None))

t, x = standardize(theta_train_raw, x_train_raw)

theta_train, x_train = t[0], x[0]


# x_o
x_o = np.array(pd.read_csv(f"./model 10/x_o_{prof}.csv", header=None))
x_o = torch.tensor((x_o - x[1]) / x[2]).float()
...
# save model
with open(f"./model 10/inference_model_10_{prof}.pkl", "wb") as handle:
