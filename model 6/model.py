...
theta_train = np.array(pd.read_csv(f"./model 3/training_theta({dataset}).csv", header=None))
x_train = np.array(pd.read_csv(f"./model 3/training_x({dataset}).csv", header=None))
...
arg = {
        ...
        "learning_rate": 0.0015121146679622883,
...
with open(f"./model 6/likelihood_estimator(model 6, v4).pkl", "wb") as handle:
    pickle.dump(likelihood_estimator, handle)
    
with open(f"./model 6/inference(model 6, v4).pkl", "wb") as handle:
