...
theta_train = torch.from_numpy(np.array(pd.read_csv("./data/training_theta(poisson).csv", header=None))).float()[:100056] 
x_train = torch.from_numpy(np.array(pd.read_csv("./data/training_x(poisson).csv", header=None))).float()[:100056]
...
import pickle
with open("./model/tune_maf(poisson).pkl", "wb") as handle:
